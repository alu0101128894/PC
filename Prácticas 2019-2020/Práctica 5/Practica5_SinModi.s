...
#PRACTICA 5 - JOSE JAVIER DIAZ GONZALEZ
	...
	.data
text:			.asciiz "Jose Javier Diaz Gonzalez\n\n"
...
reverse_r:  # funcion que da la vuelta a una cadena
		...
		#palabra = aooa
		li $s3,2
		bge $a1,$s3,no_trivial # a1 >= 2, no trivial 
		li $v0,1	# palindroma
		jr $ra		# return
		...
		no_trivial:
			...
			lb $t2,0($a0) 		# principio de la dir mem -> h en un registro
			sub $a1,$a1,1 		# a1 = a1 - 1      hola -> h ola -> 5-1=4 	10100
			add $a0,$a0,$a1 	# a1 = dir + a1		10101

			lb $t3,0($a0) 		# ultima posicion
			sb $t2,0($a0)		# guardo byte

			sub $a0,$a0,$a1  # a0 = a0 - $a1 ->
			sb $t3,0($a0)  		# t3 esta guardado
		
			beq $t3,$t2,iguales # si son iguales salta
			li $v0,0
...
###############################################
	...
	beq $t1,$s7,es_palin_i
	beq $t1,$s6,no_palin_i
	palindroma_i:

################################################


	la $a0,cadena2 		# cadena2
	li $v0,4 
	syscall
...
###############################################
	...
	beq $t3,$s7,es_palin_r # ==
	beq $t3,$s6,no_palin_r
	salir:

...
text:             .asciiz "Modificacion extra = Seleccionar filas & columnas en el menú\n"
text0:           .asciiz "\nJose Javier Diaz Gonzalez, alu0101128894\n\n"
text1:          .asciiz  "Practica 4. Trabajando con Matrices.\n"
text2:          .asciiz  "Elija opcion <0> Salir, <1> invertir fila, <2> invertir columna, <3> introducir filas & columnas: "
...
invertir_cols:
    ...
    forcols:
        ...
        bgt $t0,$s4,imprimir_matriz   # i > (ncols -1) / 2, salta do_main
...
# Si introduzco un 3 = introducir numeros
introducir_numeros:
    ...
    do_main_2:
        ...
        blt $t0,$zero,do_main_2   # seleccion < 0
        bgt $t0,$t1,do_main_2     # seleccion > 3
        
    beq $t0,0,final            
    beq $t0,1,invertir_rows_2     # si seleccion = 1 , salta invertir_rows
    beq $t0,2,invertir_cols_2
    beq $t0,3,introducir_numeros
...
# Invertir filas de la nueva matriz
invertir_rows_2:
    ...
    do_rows_2: 
        ...
        blt $t3,$t2,do_rows_2 # f < 1
        bgt $t3,$s0,do_rows_2 # f > nrows
    ...
    forrows_2:
        ...
        bgt $t1,$s4,imprimir_matriz2   # j > (ncols -1) / 2, salta imprimir_matriz
...
# Invertir columnas de la nueva matriz
invertir_cols_2:
    ...
    do_cols_2: 
        ...
        blt $t3,$t2,do_cols_2 # c < 1
        bgt $t3,$s1,do_cols_2 # c > ncols
    ...
    forcols_2:
        ...
        bgt $t0,$s4,imprimir_matriz2   # i > (ncols -1) / 2, salta do_main
...
matriz2:
    ...
    for_matriz2_1:
        bge $t0,$s0,end_matriz2_1    # i >= filas, salta end1
        addi $t0,$t0,1      # i++
        li $t1,0            # j = 0, columnas
        
        for_matriz2_2:
            bge $t1,$s1,end_matriz2_2    # j >= columnas, salta end2
